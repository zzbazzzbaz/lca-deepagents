...
状态是一个由 mail_store.py 管理的小型 JSON 文件。由 start.sh 在 langgraph dev 之前
启动，这样 make_graph() 就能在启动时发现这些工具。
"""
...
def mail_list_messages(query: str = "") -> list[dict]:
    ...
    为每条消息返回一个摘要（id、发件人、主题、日期、片段）——而不是完整正文。
    用 read_message 打开某一条。可选的 ``query`` 是一个不区分大小写的子串，
    会与主题和发件人匹配，主要是为了对应 Gmail 的搜索框；留空则列出全部。
    """
    store = load_store()
    q = query.strip().lower()
    out = []
...
def mail_create_draft(to: str, subject: str, body: str) -> dict:
    ...
    对应真实 Gmail 的 "create draft" 调用：消息被暂存起来供人工稍后审查并发送。
    在本课程中，该工具之前会运行一个人工审批环节（人在回路），因此只有在明确
    批准后才会写入草稿。
    """
    store = load_store()
    draft = {
